...
export default function HandRecognition() {
	...
	const anglesRef = useRef({ yaw: 0, pitch: 0, roll: 0 });
	...
	// Helpers (getDistance, normalizeAngle, shortestDelta, getHandEulerAngles, smoothAngles, smoothScale, drawHands)
	...
	const getHandEulerAngles = (keypoints3D: { x: number; y: number; z?: number }[]) => {
		...
		return { yaw, pitch: Math.abs(pitch) < 1e-6 ? 0 : pitch, roll: normalizeAngle(Math.abs(roll) < 1e-6 ? 0 : roll) };
	};
	const smoothAngles = (newAngles: { yaw: number; pitch: number; roll: number }, alphaScale = 1) => {
		...
		const smoothOne = (prevVal: number, newVal: number) => {
			...
			return normalizeAngle(prevVal + alpha * d);
		};
		const smoothed = {
		...
		hands.forEach((hand) => {
			...
			if (hand.handedness === "Right" && hand.keypoints3D) {
				...
				if (thumbTip && indexTip) {
					...
					const normalized = (distance - 6.0) / (12.0 - 6.0);
					const clamped = Math.max(0, Math.min(1, normalized));
					setCubeScale(smoothScale(clamped, 0.2));
			...
			if (hand.handedness === "Left" && hand.keypoints3D) {
				...
				const { yaw, pitch, roll } = smoothAngles(rawAngles, 0.2);
		...
		<>
			...
			{isTfLoaded && (
				<>
					...
					<Script src="https://cdn.jsdelivr.net/npm/@mediapipe/hands" strategy="afterInteractive" />
					<Script
