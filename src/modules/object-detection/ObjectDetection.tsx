...
export function BoundingBoxIoU() {
	...
	function render() {
		...
		ctx.fillText(
			`Score threshold: ${threshold.toFixed(2)}  |  widoczne: ${filtered.length}/${DEMO_BOXES.length}`,
			8,
			CANVAS_H - 8,
		);
	}
		...
		<div className="flex flex-col gap-4 my-6">
			<div className="flex items-center gap-4 max-w-xs">
				<Label className="whitespace-nowrap text-sm">
					Score threshold: <span className="font-mono font-semibold">{threshold.toFixed(2)}</span>
				</Label>
				<Slider
					value={[threshold]}
		...
		<div className="flex flex-col gap-4 my-6">
			<div className="flex flex-wrap gap-4 items-center">
				<div className="flex items-center gap-3 min-w-48">
					<Label className="whitespace-nowrap text-sm">
						IoU threshold: <span className="font-mono font-semibold">{iouThresh.toFixed(2)}</span>
					</Label>
					<Slider
						value={[iouThresh]}
				...
				<div className="flex flex-wrap gap-2">
					{detections.map((d, i) => (
						<span
							...
							style={{
								background: classColor(d.label) + "33",
								color: classColor(d.label),
								border: `1px solid ${classColor(d.label)}66`,
							}}
						>
