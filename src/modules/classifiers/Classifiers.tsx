...
type ClassStats = {
  mx: number; my: number; sx: number; sy: number; label: number;
};
...
function classifyBayes(lx: number, ly: number, stats: ClassStats[]): number {
  ...
  for (const s of stats) {
    ...
    if (p > bestP) { bestP = p; best = s.label; }
  }
  return best;
}
...
export function StatisticalClassifier() {
  ...
  const initialPoints = useCallback((n: number): Point[] => {
    const centers: [number, number][] = [[-0.4, -0.3], [0.4, 0.3], [-0.35, 0.4], [0.4, -0.35]];
    const result: Point[] = [];
    for (let c = 0; c < n; c++) {
  ...
  function render() {
    ...
    for (let c = 0; c < numClasses; c++) {
      ...
      if (s) { drawEllipse(ctx, s, 2); stats.push(s); }
    }
    ...
    <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-6 my-6 items-start">
      <div className="flex flex-col gap-3">
        ...
        <p className="text-sm text-muted-foreground">
          Przeciągnij <strong className="text-foreground">białe kółko</strong> po przestrzeni —
          automatycznie zmienia klasę (aktualnie:{" "}
      ...
      <div className="flex flex-col gap-4 min-w-48">
        <div>
          ...
          <ToggleGroup
            ...
            onValueChange={(v) => { if (v) setNumClasses(Number(v)); }}
          >
            {[2, 3, 4].map((n) => (
...
export function NearestCentroidClassifier() {
  ...
  const initPoints = useCallback((n: number): Point[] => {
    const centers: [number, number][] = [[-0.4, -0.3], [0.45, 0.3], [-0.3, 0.42], [0.42, -0.38]];
    const result: Point[] = [];
    for (let c = 0; c < n; c++) {
        ...
        for (let py = 0; py < H; py++) {
          for (let px = 0; px < W; px++) {
            ...
            const rgb: [number, number, number][] = [
              [59, 130, 246], [239, 68, 68], [34, 197, 94], [245, 158, 11],
            ];
    ...
    <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-6 my-6 items-start">
      <div className="flex flex-col gap-3">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          {Math.sqrt(
            (testPt.x - (centroids[currentLabel]?.x ?? 0)) ** 2 +
              (testPt.y - (centroids[currentLabel]?.y ?? 0)) ** 2,
          ).toFixed(3)}
        </p>
      </div>
      ...
      <div className="flex flex-col gap-4 min-w-48">
        ...
        <div>
          ...
          <ToggleGroup
            ...
            onValueChange={(v) => { if (v) setShowRegions(v === "on"); }}
          >
        ...
        <Button
          variant="outline"
          size="sm"
          onClick={() => { setPts(initPoints(numClasses)); setTestPt({ x: 0.05, y: 0.05, label: 0 }); regionCache.current = null; }}
        >
          <RotateCcw className="w-4 h-4 mr-2" /> Reset
        </Button>
      </div>
...
export function LinearClassifier() {
  ...
  const initPoints = useCallback((): Point[] => [
    ...randn2(-0.45, -0.3, 0.22, 0.2, 30, 0),
    ...randn2(0.45, 0.3, 0.22, 0.2, 30, 1),
  ], []);

  useEffect(() => { setPts(initPoints()); }, [initPoints]);
  useEffect(() => { render(); }, [pts, angle, offset]);
  ...
  function render() {
    ...
    // Background regions
    const id = ctx.createImageData(W, H);
    const rgbs: [number, number, number][] = [[59, 130, 246], [239, 68, 68]];
    for (let py = 0; py < H; py++) {
      for (let px = 0; px < W; px++) {
        const [lx, ly] = toLogical(px, py);
        ...
        const off = (py * W + px) * 4;
        id.data[off] = r; id.data[off + 1] = g; id.data[off + 2] = b; id.data[off + 3] = 35;
      }
    }
    ctx.putImageData(id, 0, 0);
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-6 my-6 items-start">
      <div className="flex flex-col gap-3">
        ...
        />
        <p className="text-sm text-muted-foreground">
          Ustaw granicę decyzyjną suwakami.{" "}
          <span className="font-semibold text-foreground">{errors}</span> z{" "}
          <span className="font-semibold text-foreground">{pts.length}</span> punktów jest źle
          sklasyfikowanych.
        </p>
      </div>
...
export function PiecewiseLinearClassifier() {
  ...
  const initPoints = useCallback((): Point[] => [
    ...randn2(-0.45, 0.1, 0.28, 0.35, 35, 0),
    ...randn2(0.48, -0.05, 0.28, 0.35, 35, 1),
  ], []);

  useEffect(() => { setPts(initPoints()); }, [initPoints]);
  useEffect(() => { render(); }, [pts, segs, hoveredHandle]);
    ...
    if (poly.length > 2) {
      ...
      for (let py = 0; py < H; py++) {
        for (let px = 0; px < W; px++) {
          ...
          const off = (py * W + px) * 4;
          id.data[off] = r; id.data[off + 1] = g; id.data[off + 2] = b; id.data[off + 3] = 38;
        }
      }
      ctx.putImageData(id, 0, 0);
    }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-6 my-6 items-start">
      <div className="flex flex-col gap-3">
        ...
        />
        <p className="text-sm text-muted-foreground">
          Przeciągaj <strong className="text-foreground">białe uchwyty</strong> na granicy
          decyzyjnej. Błędy klasyfikacji:{" "}
          <span className="font-semibold text-foreground">{errors}</span>/{pts.length}.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-4 min-w-48">
        ...
        <Button
          variant="outline"
          size="sm"
          onClick={() => { setPts(initPoints()); setSegs(INITIAL_SEGS); }}
        >
          <RotateCcw className="w-4 h-4 mr-2" /> Reset
        </Button>

        <div className="text-sm text-muted-foreground flex flex-col gap-1">
          <span>Segmentów: <strong className="text-foreground">{segs.length}</strong></span>
